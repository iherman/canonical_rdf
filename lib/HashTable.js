...
class HashBag {
     ...
     */
    constructor(hashes, debug = false) {
    ...
    /**
     * Calculate the hash value related to the blank node 'b'. The value is calculated using the hashTerm function
     * on the accumulated values, previously ordered.
     *
     * @param b: blank node identifier
    ...
    value(b) {
        assert.ok(this._values[b] !== undefined);
        let sorted_vals = _.map(this._values[b].sort(HashValue.hashCompare), (h) => h.value)
        let retval = hashTerm(Buffer.concat(sorted_vals));
        return retval;
    }
    ...
    /* ====================== */
    toString() {
        return JSON.stringify(this._values, null, 4);
    }
}
