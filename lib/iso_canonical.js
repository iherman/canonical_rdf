    ...
    if( hash === undefined ) {
        ...
        _.forEach(G.terms, (term) => {
            if( n3util.isBlank(term) ) {
                hash.setHash(term, hashTerm())
            } else {
                hash.setHash(term, hashTerm(term))
            }
        });
    }
 ...
 * does the order already, which makes this unnecessary as a separate step.
...
function distinguish(G, hash, Gmin) {
    ...
    _.forEach(P[index], (b) => {
        ...
        if( hash_prime.isTrivial() ) {
            // We may found a solution, if it is smaller than Gmin
