...
const { HashValue, hashTerm, hashTuple}  = require('./Hash');
const { HashTable, HashBag }             = require('./HashTable');
 ...
 * This is "Algorithm 1" in the paper, modified on the hash bag aspect (see Readme for further details)
 *
 * @param G: a Dataset
...
function hashBnodes(G, hash) {
    ...
    if(debug) console.log("---- Starting hash Bnodes algorithm")
    ...
    do {
        ...
        hashIm1  = hashI.clone();
        hashI    = hashIm1.clone();
        hash_bag = new HashBag(hashI, debug);
 ...
 *
 * @param G: a Dataset
 * @param run_with_debug: whether the run should be done in debug mode
 * @returns: a Dataset with canonical renames
 */
function isoCanonicalize(G, run_with_debug = false) {
    ...
    if( hash.isTrivial() ) {
        ...
        if(debug) console.log("---- Graph is trivial")
